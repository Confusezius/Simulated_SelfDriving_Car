...
class nvidia_net(nn.Module):
    def __init__(self, kernel_sizes=[5,5,5,3,3], image_size=[3,160,320], n_filters=[24, 36, 48, 64, 64], use_BN=False,\
                 fc_layers=[500, 100, 50, 10], conv_dropout=[0.3,0.2,0.2,0.2,0], fc_dropout=[0.3,0.2,0.2,0], make_full_conv=False,\
                 strides = [2,2,2,2,1], n_channels=3, conv_activation=nn.ReLU(), fc_activation=nn.ReLU()):
        ...
        assert n_channels%3==0, "Number of channels must be multiples of 3."
        ...
        conv_layer_list = []
        fc_layer_list   = []
        self.n_filters  = [n_channels]+n_filters
